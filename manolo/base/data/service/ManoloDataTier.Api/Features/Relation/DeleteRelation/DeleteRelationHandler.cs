...
public class DeleteRelationHandler : IRequestHandler<DeleteRelationQuery, Result>{
    ...
    public DeleteRelationHandler(ManoloDbContext context, IIdResolverService idResolverService, ServiceHelpers serviceHelpers){
        _context           = context;
        _idResolverService = idResolverService;
        _serviceHelpers    = serviceHelpers;
    }

    public async Task<Result> Handle(DeleteRelationQuery request,
        ...
        var result = await _context.Relations
                                   .Where(d => d.Subject == subjId && d.Object == objId && d.Predicate == existingPredicate.Id)
                                   .ExecuteDeleteAsync(cancellationToken);

        return result == 0
            ? Result.Failure(DomainError.RelationDoesNotExist(subjId, objId, request.Predicate))
            : Result.Success(subjId);
    }
