...
var port = int.TryParse(Environment.GetEnvironmentVariable("HTTP3_PORT"), out var p) ? p : 5001;
...
// Add DbContext
builder.Services.AddDbContext<ManoloDbContext>(options =>
...
builder.Services.AddAuthorizationBuilder()
       .AddPolicy("AdminOnly", policy =>
                      policy.RequireAssertion(context => {
                          var accessLevelClaim = context.User.FindFirst("AccessLevel")?.Value;
       ...
       .AddPolicy("ModeratorOrHigher", policy =>
                      policy.RequireAssertion(context => {
                          var accessLevelClaim = context.User.FindFirst("AccessLevel")?.Value;
