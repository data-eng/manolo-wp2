...
public class DeleteDataStructureHandler : IRequestHandler<DeleteDataStructureQuery, Result>{

    private readonly ManoloDbContext    _context;
    private readonly IIdResolverService _idResolverService;

    public DeleteDataStructureHandler(ManoloDbContext context, IIdResolverService idResolverService){
        _context           = context;
        _idResolverService = idResolverService;
    }

    public async Task<Result> Handle(DeleteDataStructureQuery request,
        ...
        existingDataStructure.IsDeletedRaw = 1;
